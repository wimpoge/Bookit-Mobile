...
class _AnalyticsReportsScreenState extends State<AnalyticsReportsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAnalyticsData() async {
    ...
    try {
      ...
      // Load checkout performance
      final checkoutResponse = await apiService.get('/analytics/checkout-performance?period=$periodParam');

      // Load guest lifecycle
      final lifecycleResponse = await apiService.get('/analytics/guest-lifecycle?period=$periodParam');
      ...
      if (mounted) {
        setState(() {
          _analyticsData = {
            ...
            'checkout_performance': checkoutResponse,
            'guest_lifecycle': lifecycleResponse,
          };
...
Generated by BookIt Analytics
  ...
  Widget _buildCheckoutTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionTitle('Checkout Performance'),
          const SizedBox(height: 16),
          _buildCheckoutMetrics(),

          const SizedBox(height: 24),

          _buildSectionTitle('Checkout Trends by Day'),
          const SizedBox(height: 16),
          _buildCheckoutTrendsChart(),

          const SizedBox(height: 24),

          _buildSectionTitle('Checkout Timing Analysis'),
          const SizedBox(height: 16),
          _buildCheckoutTimingChart(),
        ],
      ),
    );
  }
  ...
  Widget _buildLifecycleTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionTitle('Guest Journey Funnel'),
          const SizedBox(height: 16),
          _buildBookingFunnel(),
          ...
          _buildSectionTitle('Revenue Completion'),
          const SizedBox(height: 16),
          _buildRevenueCompletion(),
        ],
      ),
    );
  }

  Widget _buildCheckoutMetrics() {
    final checkoutData = _analyticsData['checkout_performance'];
    final metrics = checkoutData?['checkout_metrics'] ?? {};
    ...
    return GridView.count(
      ...
      children: [
        ...
        _buildKPICard(
          'Checkout Revenue',
          '\$${(metrics['checkout_revenue'] ?? 0.0).toStringAsFixed(2)}',
          '',
        ...
        _buildKPICard(
          'Avg Stay Duration',
          '${(metrics['avg_stay_duration'] ?? 0.0).toStringAsFixed(1)} days',
          '',
        ...
        _buildKPICard(
          'Avg Rating',
          '${(metrics['avg_rating'] ?? 0.0).toStringAsFixed(1)} ⭐',
  ...
  Widget _buildCheckoutTrendsChart() {
    final checkoutData = _analyticsData['checkout_performance'];
    final trends = checkoutData?['checkout_trends'] ?? [];
    ...
    if (trends.isEmpty) {
      return Container(
        ...
        child: Center(
          child: Text('No checkout data available', style: GoogleFonts.poppins()),
        ),
      );
    }
        ...
        BarChartData(
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  if (value.toInt() < trends.length) {
                    return Text(
                      trends[value.toInt()]['day'].substring(0, 3),
                      style: GoogleFonts.poppins(fontSize: 12),
                    );
                  }
                  return const Text('');
          ...
          barGroups: trends
            ...
            return BarChartGroupData(
              ...
              barRods: [
                BarChartRodData(
                  toY: entry.value['checkouts'].toDouble(),
                  color: Colors.green,
                  width: 20,
                  borderRadius: const BorderRadius.only(
                    topLeft: Radius.circular(4),
  ...
  Widget _buildCheckoutTimingChart() {
    final checkoutData = _analyticsData['checkout_performance'];
    final timing = checkoutData?['checkout_timing'] ?? {};

    final early = timing['early_checkouts'] ?? 0;
    final onTime = timing['on_time_checkouts'] ?? 0;
    final late = timing['late_checkouts'] ?? 0;
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              _buildLegendItem('Early (<11am)', Colors.blue, early),
              _buildLegendItem('On Time', Colors.green, onTime),
              _buildLegendItem('Late (>12pm)', Colors.red, late),
            ],
          ),
        ],
      ),
  ...
  Widget _buildBookingFunnel() {
    final lifecycleData = _analyticsData['guest_lifecycle'];
    final funnel = lifecycleData?['booking_funnel'] ?? {};
    ...
    return Container(
      ...
      child: Column(
        children: [
          _buildFunnelStep('Total Bookings', funnel['total_bookings'] ?? 0, 1.0, Colors.blue),
          _buildFunnelStep('Confirmed', funnel['confirmed_bookings'] ?? 0, 0.8, Colors.green),
          _buildFunnelStep('Checked In', funnel['checked_in'] ?? 0, 0.6, Colors.orange),
          _buildFunnelStep('Checked Out', funnel['checked_out'] ?? 0, 0.4, Colors.purple),
          const SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              _buildStatusCount('Cancelled', funnel['cancelled'] ?? 0, Colors.red),
              _buildStatusCount('No Shows', funnel['no_shows'] ?? 0, Colors.grey),
            ],
          ),
        ],
      ),
  ...
  Widget _buildConversionRates() {
    final lifecycleData = _analyticsData['guest_lifecycle'];
    final rates = lifecycleData?['conversion_rates'] ?? {};
    ...
    return GridView.count(
      ...
      children: [
        _buildKPICard(
          'Confirmation Rate',
          '${(rates['confirmation_rate'] ?? 0.0).toStringAsFixed(1)}%',
          '',
          Icons.check_circle,
          Colors.green,
        ),
        _buildKPICard(
          'Check-in Rate',
          '${(rates['checkin_rate'] ?? 0.0).toStringAsFixed(1)}%',
          '',
          Icons.login,
          Colors.blue,
        ),
        _buildKPICard(
          'Completion Rate',
          '${(rates['completion_rate'] ?? 0.0).toStringAsFixed(1)}%',
          '',
          Icons.flag,
          Colors.purple,
        ),
  ...
  Widget _buildRevenueCompletion() {
    final lifecycleData = _analyticsData['guest_lifecycle'];
    final revenue = lifecycleData?['revenue_tracking'] ?? {};

    final confirmedRevenue = revenue['total_revenue_confirmed'] ?? 0.0;
    final completedRevenue = revenue['total_revenue_completed'] ?? 0.0;
    final completionRate = revenue['revenue_completion_rate'] ?? 0.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildRevenueItem('Confirmed Revenue', confirmedRevenue, 1.0),
          _buildRevenueItem('Completed Revenue', completedRevenue, completionRate / 100),
          const SizedBox(height: 16),
          ...
          const SizedBox(height: 8),
          Text(
            'Revenue Completion Rate: ${completionRate.toStringAsFixed(1)}%',
            style: GoogleFonts.poppins(
              fontSize: 14,
              fontWeight: FontWeight.w500,
              color: Colors.green,
  ...
  Widget _buildFunnelStep(String label, int value, double width, Color color) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(
        children: [
